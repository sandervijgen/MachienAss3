...
def placeBin(x, y, unWantedness):  # quantification of how inappropriate it is for the robot to disturb at this location
    MAP[y][x] = "X"
...
def find_shortest_paths_from_bins(robot_loc, bin_list, map):
    for index, bin_loc in enumerate(bin_list, start=1):
        bin_x, bin_y, unWantedness = bin_loc

        # Creating a new node for each bin
        node_name = f'Bin_{index}'
        G.add_node(node_name, pos=(bin_x, bin_y))

        shortest_path_length = find_shortest_path(robot_loc, (bin_x, bin_y), map)
        G.add_edge('Robot', node_name, weight=shortest_path_length)
        print(f"The shortest path length from the starting position to {node_name} is: {shortest_path_length}")

    # Calculate paths between mutual elements in BIN_LIST
    for i in range(len(bin_list)):
        for j in range(i + 1, len(bin_list)):
            node_name_i = f'Bin_{i + 1}'  # Get node name for bin i
            node_name_j = f'Bin_{j + 1}'  # Get node name for bin j
            shortest_path_length = find_shortest_path(bin_list[i][:2], bin_list[j][:2], map)
            G.add_edge(node_name_i, node_name_j, weight=shortest_path_length)
            print(f"The shortest path length between {node_name_i} and {node_name_j} is: {shortest_path_length}")
...
nt.from_nx(G)
# Adjust edge width
nt.set_edge_smooth('dynamic')  # Ensures edge width changes smoothly
